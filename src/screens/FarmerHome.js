...
const IndustryListing = ({ navigation }) => {
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            {filterCategories.map((category) => (
              ...
              >
                <Text style={[styles.optionText, selectedFilters.includes(category) && styles.selectedText]}>{category}</Text>
              </TouchableOpacity>
            ))}
...
const styles = StyleSheet.create({
  ...
  searchContainer: { flexDirection: 'row', backgroundColor: '#f1f1f1', borderRadius: 8, padding: 8, alignItems: 'center', marginBottom: 10 },
  searchIcon: { marginRight: 8 },
  searchInput: { flex: 1 },
  tabContainer: { flexDirection: 'row', marginBottom: 10 },
  tab: { fontSize: 16, marginRight: 20, color: '#888' },
  activeTab: { fontWeight: 'bold', color: 'green', borderBottomWidth: 2, borderBottomColor: 'green' },
  card: { flexDirection: 'row', backgroundColor: '#f9f9f9', padding: 10, borderRadius: 8, marginBottom: 10 },
  cardText: { flex: 1 },
  cardTitle: { fontSize: 16, fontWeight: 'bold' },
  cardDemand: { fontSize: 14, color: '#555' },
  cardDescription: { fontSize: 12, color: '#777', marginBottom: 6 },
  connectButton: { borderColor: 'green', borderWidth: 1, borderRadius: 6, padding: 6, width: 80, alignItems: 'center' },
  connectButtonText: { color: 'green', fontSize: 14 },
  bottomNav: { flexDirection: 'row', justifyContent: 'space-around', padding: 12, borderTopWidth: 1, borderTopColor: '#ccc', backgroundColor: '#fff' },
  ...
  closeButton: { marginTop: 20, backgroundColor: 'green', padding: 10, borderRadius: 6, alignItems: 'center' },
